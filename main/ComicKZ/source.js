...
var _Sources = (() => {
  ...
  // src/ComicKZ/ComicKZ.ts
  var ComicKZ_exports = {};
  __export(ComicKZ_exports, {
    ComicKZ: () => ComicKZ,
    ComicKZInfo: () => ComicKZInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/ComicKZ/parser.ts
  ...
  var Parser = class {
    parseMangaDetails(data, mangaId) {
      ...
      if (genres.length > 0) {
        const tags = genres.map(
          (g) => App.createTag({ id: g.slug ?? "", label: g.name ?? "" })
        );
        tagSections.push(
      ...
      return App.createSourceManga({
        ...
        mangaInfo: App.createMangaInfo({
          ...
          author,
          artist: author,
          rating: data.bayesian_rating ? Number(data.bayesian_rating) : 0,
          contentRating
        })
      });
    }
      ...
      for (const item of data) {
        ...
        chapters.push(
          App.createChapter({
            ...
            langCode: "en",
            time: item.created_at ? new Date(item.created_at) : /* @__PURE__ */ new Date(0),
    ...
    parseSearchResults(data) {
      ...
      for (const item of data) {
        ...
        const subtitle = item.last_chapter ? `Ch. ${item.last_chapter}` : "";
        results.push(
    ...
    parseHomeSections(popularData, recentData, updateData, sectionCallback) {
  ...
  // src/ComicKZ/ComicKZ.ts
  var BASE_URL = "https://comickz.co.uk";
  var ID_SEP2 = "|#|";
  var ComicKZInfo = {
    version: "1.0.0",
    name: "ComicKZ",
  ...
  var ComicKZ = class {
    ...
    getMangaShareUrl(mangaId) {
      const slug = mangaId.split(ID_SEP2)[1];
    ...
    async fetchHomeSection(section) {
      let queryParams;
      switch (section) {
        case "popular":
          queryParams = {
  ...
  return __toCommonJS(ComicKZ_exports);
})();
